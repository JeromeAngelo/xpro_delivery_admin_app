...
abstract class DeliveryDataRemoteDataSource {
  ...
  Future<List<DeliveryDataModel>> searchDeliveryData(String query);
}
...
class DeliveryDataRemoteDataSourceImpl implements DeliveryDataRemoteDataSource {
    ...
    try {
      ...
      try {
        ...
        if (invoiceStatusRecords.isNotEmpty) {
          ...
          debugPrint(
            '✅ Deleted ${invoiceStatusRecords.length} invoiceStatus records linked to deliveryData: $id',
          );
        } else {
          debugPrint(
            'ℹ️ No invoiceStatus records found referencing deliveryData: $id',
          );
        }
      } catch (e) {
        debugPrint(
      ...
      if (invoiceId != null && invoiceId.toString().isNotEmpty) {
        ...
        try {
          ...
          debugPrint(
            '⚠️ Failed to delete linked invoice record: ${e.toString()}',
          );
        }
      }
  ...
  DeliveryDataModel _processDeliveryDataRecordFull(RecordModel record) {
    ...
    if (deliveryUpdateRecords.isNotEmpty) {
      deliveryUpdatesList = deliveryUpdateRecords
          .map((update) {
    ...
    if (invoiceItemsRecords.isNotEmpty) {
      invoiceItemsList = invoiceItemsRecords
          .map((item) {
